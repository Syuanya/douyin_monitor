...
当前版本：`1.0.5`。本版本继续优化任务中心与下载历史：新增统一运行状态模型，规范任务/下载/文件状态映射，强化任务与下载记录关联查询，保留旧恢复记录兼容层，并让桌面 UI、服务层和 Web API 后续都能基于稳定状态 key 扩展。GitHub 包会清理缓存、日志、运行数据、测试缓存和敏感本地配置，更适合直接提交源码仓库。
...
- 下载管理：提供下载队列、任务中心、下载历史、失败原因提示、重试入口、恢复任务进度和文件状态校验。
